...
namespace Mobile_ecommerce.Areas.Admin.Controllers
{
    public class UserController : BaseController
    {
        ...
        {
            var user = await userdb.GetById(id);
            ViewBag.ListRole = await userdb.GetAllRole();
            return View(user);
        }

        [HttpPost]
        public async Task<ActionResult> Edit(Edit user)
        {
            if (ModelState.IsValid)
            {
                var result = await userdb.UpdateUser(user);
                if (result)
                {
        ...
        {
            var result = await userdb.Delete(userId);
            if (result)
            {
        ...
        {
            var user = await userdb.GetRoleById(id);
            ViewBag.ListRole = await userdb.GetAllRole();
            return View(user);
        }
        ...
        public async Task<ActionResult> RoleAssign(UserRole userRole)
        {
            if (ModelState.IsValid)
            {
                var result = await userdb.UpdateRole(userRole);
                if (result)
                {
